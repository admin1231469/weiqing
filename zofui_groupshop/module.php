...
class Zofui_groupshopModule extends WeModule {
	...
	public function settingsDisplay($settings) {
		...
		load()->func('tpl');
		load()->model('account');
		$settings['exname'] = iunserializer( $settings['exname'] );
		$settings['addparams'] = iunserializer( $settings['addparams'] );
		...
		if(checksubmit()) {
			...
			$dat = array(
				...
				'cratfreeex' => $_GPC['cratfreeex'],
				
				'kdniaoid' => $_GPC['kdniaoid'],
				'kdniaokey' => $_GPC['kdniaokey'],
				'ispay' => intval( $_GPC['ispay'] ),
				'isverify' => intval( $_GPC['isverify'] ),
				'isindexverify' => intval( $_GPC['isindexverify'] ),
				'isptime' => intval( $_GPC['isptime'] ),
				'itfee' => $_GPC['itfee'],
				'exfee' => $_GPC['exfee'],
				'extype' => $_GPC['extype'],
				
				'a_id'=>$_GPC['a_id'],//收徒成功模板 isindexverify
				'a_item'=>$_GPC['a_item'],
				...
				'issendpay' => $_GPC['issendpay'],	
				
            );

			if( !empty( $_GPC['exname'] ) ) {
				$dat['exname'] = iserializer( $_GPC['exname'] );
			}
			
			$rule = array();
			if( !empty( $_GPC['paramsadd'] ) ) {
				foreach ((array)$_GPC['paramsadd'] as $k => $v) {
					$ruleitem =array();
					$ruleitem['num'] = $_GPC['paramsadd'][$k];
					$ruleitem['give'] = $_GPC['paramsgive'][$k];

					$rule[] = $ruleitem;
				}
			}
			$dat['addparams'] = iserializer( $rule );
...
pdo_query("CREATE TABLE IF NOT EXISTS ".tablename('zofui_groupshop_addorder')." (
