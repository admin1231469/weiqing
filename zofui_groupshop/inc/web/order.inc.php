<?php 
	...
	$exname = iunserializer( $this->module['config']['exname'] );
	...
	if(checksubmit('import')) {
		...
		foreach($result as $k => $v){
			if($k >= 1){ //第0个是上面的标题栏
				...
				$data['expressnumber'] = str_replace('a','', $data['expressnumber']);			
	...
	function returnfile( $file ){
		return 	$file['orderfile']['name'];
	}

	function returntemp( $file ){
		return 	$file['orderfile']['tmp_name'];
	}	
	...
	if($op == 'list'){
		...
		$res = WebCommon::slelectOrder($wherea,$whereb,$str,$_GPC['page'],30,$order,$by);
	...
	if( $op == 'comorder' ){
		$res = pdo_update('zofui_groupshop_order',array('status'=>5,'confirmtime'=>TIMESTAMP),array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['id'],'status'=>3));

		$orderinfo = pdo_get('zofui_groupshop_order',array('id'=>$_GPC['id']));
		...
		Message::hmessage($orderinfo['openid'],$this,$orderinfo['orderid'],$orderinfo['id']);
	...
	function downLoadOrder($wherea,$whereb,$str,$order,$by){
		...
		$html .= '用户留言' . "\t,";
		$html .= '快递名称' . "\t,";
		$html .= '快递单号(若保存后出现数值不对，可在单号前加一个a)' . "\t,";
		$html .= '支付时间' . "\t,";
		$html .= '管理员备注' . "\t,";
		$html .= "\n";
 		...
 		foreach((array)$list as $k => $v){	
			foreach($v as $kk => $vv){
				...
				$paytype = '微信支付';
				if( $vv['paytype'] == 'credit' ) $paytype = '余额支付';
				if( $vv['paytype'] == 'delivery' ) $paytype = '货到付款';
