<?php 
	...
	if($_GPC['op'] == 'location'){
		...
		}elseif($_GPC['type'] == 'cancela'){  //取消未支付的订单

			if( $orderinfo['paytype'] != 'delivery' ) die;
...
div;
		...
		$allgood = pdo_getall('zofui_groupshop_ordergood',array('idoforder'=>$order['id']));
		
		foreach ($allgood as $k => $v) {
			$credit = pdo_get('zofui_groupshop_good',array('id'=>$v['gid']),'usercredit');
			//个人积分奖励
			if($credit['usercredit'] > 0){
				model_user::updateUserCredit($order['openid'],$credit['usercredit']*$v['buynum'],4);
			}
		}

		Message::hmessage($order['openid'],$this,$order['orderid'],$order['id']);

		Util::deleteCache( 'order',$order['id'] );
		
		Util::echoResult(200,'订单已核销完成');		

	}elseif( $_GPC['op'] == 'verifyadd' ){

		if( $userinfo['verify'] != 0 ) Util::echoResult(201,'不能再提交');
		...
		if( empty( $data['name'] ) ) Util::echoResult(201,'请填写姓名');
		if( empty( $data['tel'] ) ) Util::echoResult(201,'请填写电话');
		if( empty( $data['street'] ) ) Util::echoResult(201,'请填写街道');	

		$params = iserializer( $data );	

		pdo_update('zofui_groupshop_user',array('verify'=>1,'params'=>$params),array('uniacid'=>$_W['uniacid'],'id'=>$userinfo['id']));
		Util::deleteCache( 'fsuser',$userinfo['openid'] );
