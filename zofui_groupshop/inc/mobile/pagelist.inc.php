...
div;
	...
	if(in_array($_GPC['op'],array('goodcomment'))){
		...
		foreach($data[0] as $k => $v){
			...
			if( empty($this->module['config']['isptime']) ) {
...
div;
	...
	if(in_array($_GPC['op'],array('order'))){
		...
		$select = ' a.id,a.orderid,a.ordertype,a.totalmoney,a.totalnum,a.status,a.refundstatus,b.gid,b.pic,b.title,b.rule,b.buynum,b.buymoney,b.iscomment,b.idoforder,a.paytype ';
				...
				foreach($v as $kk => $vv){
					...
					if($vv['ostatus'] == 1){
						...
						if($vv['ordertype'] == 1 &&  $this->module['config']['iscanrefund'] == 1 && $vv['refundstatus'] == 0){
							...
							if( $vv['paytype'] == 'delivery' ){
								$dealstr = '<a href="javascript:;" data-id="'.$vv['id'].'"><span class="fr order_btn_deal" data-type="cancela">取消订单</span></a>';
							}else{
