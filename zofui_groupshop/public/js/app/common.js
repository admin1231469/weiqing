	...
	*/
	...
	common.createUrl = function(dostr,opstr,obj){
		...
		return window.sysinfo.siteroot+'app/index.php?i='+window.sysinfo.uniacid+'&c=entry'+str+'&m=zofui_groupshop';
	};
	...
	common.getPage = function(params,succall,comcall){
		...
		common.Http('post','json',common.createUrl('pagelist',params.op),params,function(data){
			...
			if( initParams.issetpage == 1 )  detail2listobj.insertHtmlStr($(".insertHtmlStr").html(),params);
	...
	common.indexInit = function(){
        ...
        $('body').on('click','.kefutype',function(){
			var type = $(this).attr('kefutype');
			var tel = $(this).attr('kefutel');
         ...
         $('body').on('click','.kefuqr_box .mask',function(){
        	$('.kefuqr_box').hide();
        });
