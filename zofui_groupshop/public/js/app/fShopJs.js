  ...
  'use strict';
	...
	var good = {
		...
		init: {
			indexinit : function(){ 
				...
				common.hideActionSheet($('#sideup_gooddetail #weui_actionsheet'), $('#sideup_gooddetail #mask') );
				good.fn.changeDetailItem($('.detail_body_bot'));
			}
		},
		events:{
			'.detail_btn' : { //显示相应详情
			...
			'#good_foot_back' : { //返回
				click : function(){
					var aa = window.history.go(-1);
					if( !aa ){
						location.href = common.createUrl('index','index');
					}
				}
			},
		...
		fn:{
			...
			getDetailContent : function(showtype,elemt){ //加载内容
				if(showtype == 'detail_body' && $('.detail_body').html() == ''){
					//var content = common.htmlspecialchars_decode($('input[name=detail_content]').val());
					console.log( initParams.detail );
					$('.detail_body').html(initParams.detail);
				}
	...
	var sort = {
		init: {
			initfun : function(){
				common.goToTop(); //回到顶部
				sort.fn.sortPic($('.sortright_one').first());//第一个分类图片
			}
		},		
		events:{	
			'.sortleft li' : {
				click : function(){
					...
					var thisclass = $(this);
					var thisitem = thisclass.attr('data-index');
					thisclass.parent().find('.activity').removeClass('activity');
					thisclass.addClass('activity');
					$('#sortitem_' + thisitem).show().siblings().hide();	
					if(thisclass.offset().top > $(window).height()-150)
						$('.sortleft').scrollTop($('.sortleft').scrollTop()+200);
					if(thisclass.offset().top < 150)
						$('.sortleft').scrollTop($('.sortleft').scrollTop()-200);
					sort.fn.sortPic($('#sortitem_' + thisitem));//分类图片
				}
			},	
		},
		fn : {
			sortPic : function(elemt){ //分类图片	
				elemt.find('img').each(function(){
		...
		events:{	
			'#checkcode' : {
				click : function(){
					...
					common.Http('post','json',common.createUrl('ajaxdeal','hexiao1'),postdata,function(data){
						if( data.status == 200 ){
			...
			'#comcheckcode' : { 
				click : function(){
					var postdata = {
						id : initParams.orderid,
					};
					common.Http('post','json',common.createUrl('ajaxdeal','hexiao2'),postdata,function(data){
						if( data.status == 200 ){
		...
		events:{	
			'#sub' : {
				click : function(){
					...
					if( postdata.tel == '' || !common.verify('mobile',postdata.tel) ){
						common.alert('填写正确的手机号码');return false;
					}

					common.Http('post','json',common.createUrl('ajaxdeal','verifyadd'),postdata,function(data){
						if( data.status == 200 ){
							location.href = "";
						}else{
							common.alert(data.res);
		...
		events:{
			...
			'input[name=paygood]' : {
				click : function(){
					...
					if( (address == undefined || address == '' ) && sendtype == '0' ){
						common.alert('您还没有选择收货地址');return false;
					}
					if( sendtype == 1 && ( mobile == '' || !common.verify('mobile',mobile) ) ){
						common.alert('填写正确的联系电话');return false;
					}
				}
			},
			'input[name=sendtype]' : {
		...
		fn:{
			changeParamsFunc : function(cardid){ //选择地址，卡券等重置参数
				...
				common.Http('post','json',common.createUrl('ajaxdeal','selectcard'),postdata,function(data){
					if(data.status == 1){
						$('#total_fee').text( data.totalmoney[1].toFixed(2) ); //总金额
							...
							for(var t in data.goodinfo){
								if(data.goodinfo[t].id == id){
									if(data.goodinfo[t].buyexpressmoney*1 > 0){
										...
										thisclass.find('.good_item_express,.good_item_expressa').text(data.goodinfo[t].buyexpressmoney);
									}else{
									...
									var nn = data.goodinfo[t].buynumber*data.goodinfo[t].buyprice + data.goodinfo[t].buyexpressmoney*1;

									thisclass.find('.good_item_toal').text( nn.toFixed(2) );
								}
								
							}
						});
						
						if( data.sendtype == 0 ){ // 物流
							$('.confirm_address').css({'display':'-webkit-box'});
							$('.confirm_shopinfo,.confirm_mobile').hide();
						}else if( data.sendtype == 1 ){ // 自提
							$('.confirm_address').hide();
							$('.confirm_shopinfo,.confirm_mobile').show();
						}
		...
		events:{
			...
			'.order_btn_deal' : { //处理订单
				click : function(){
					...
					if(postdata.type == 'cancela') var notice = '确定要取消订单吗？';
	...
	//confirm页面
	$(document).on("pageInit", "#page_hexiao", function(e, id, page) {
		common.init(hexiao);
	});	

	//verify
	$(document).on("pageInit", "#page_verify", function(e, id, page) {
		common.init(verify);
	});
	...
	$(document).on("pageInit", "#page_user,#page_orderinfo", function(e, id, page) {
		...
		if($.inArray(initParams.op,['order']) >= 0) { //订单列表
			...
			if(config.user.status >= 4 || detail2listobj.getLocalStorage()['extraData']['status'] >= 4 ){ //选择最右边的状态时，将滚动条滚动到最右边
	...
	$(document).on("pageInit", "#page_index", function(e, id, page) {
		...
		$(".content").scroll(function(){
			detail2listobj.insertHtmlStr($(".insertHtmlStr").html(),{});
		});
	...
	$.init();
	common.Http('post','json',common.createUrl('ajaxdeal','queue'),{},function(data){
		
	},'',true);	


	$('body').off('click','#gotoback').on('click','#gotoback',function(){
    ...
    window.addEventListener("popstate", function(e) {  
        if(bool && document.referrer.indexOf('do=index') > 0 ){
            location.href = common.createUrl('index','index');
            pushHistory();
        }
    }, false);
