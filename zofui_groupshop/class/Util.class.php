	...
	*/
	...
	//查询缓存
 	static function getCache($key,$name) {
		...
		return cache_read('zfgp:'.$_W['uniacid'].':'.$key.':'.$name);
	}
	...
	static function setCache($key,$name,$value) {
		...
		$res = cache_write('zfgp:'.$_W['uniacid'].':'.$key.':'.$name,$value);
		return $res;
	}
	...
	static function deleteCache($key,$name) {
		global $_W;		
		if(empty($key) || empty($name)) return false;
		cache_write('zfgp:'.$_W['uniacid'].':'.$key.':'.$name,'');
		return cache_delete('zfgp:'.$_W['uniacid'].':'.$key.':'.$name);
	}
