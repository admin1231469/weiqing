...
class model_express
{
	...
	static function getAllExpress(){
		return Util::getDataByCacheFirst('express','allexpress',array('Util','getAllDataInSingleTable'),array('zofui_groupshop_express',array('uniacid'=>$_W['uniacid']),1,9999,'`id` DESC'));
		//需删除缓存
	}
	...
	static function countExpressMoney($goodinfo,$buynum,$province){
		...
		if($goodinfo['expresstype'] == 0){		
			...
			if( empty( $goodinfo['ismaxexpress'] ) ){
				$expressmoney = $goodinfo['expressmoney'];
			}else{
	...
	static function decodeExpress($expressname){
		if( !(strpos($expressname,'顺丰') === false) ){
			...
			$str = 'DBL';			
		}elseif( !(strpos($expressname,'百世') === false)){
			$str = 'HTKY';			
		}elseif( !(strpos($expressname,'优速') === false)){
			$str = 'UC';			
		}elseif( !(strpos($expressname,'邮政') === false)){
			$str = 'YZPY';			
		}
		return	$str;
	}	
