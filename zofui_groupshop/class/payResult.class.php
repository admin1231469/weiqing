		...
		if($params['result'] == 'success' && $params['from'] == 'notify'){
			...
			if( !empty( $orderinfo ) ) {
				...
				if($orderinfo['status'] == 1  && $orderinfo['totalmoney'] == $params['fee']){
					...
					self::dealOrder($orderinfo,$params,$module,1);
...
/*					//是新建团单 先获取团id
				...
				$orderinfo = pdo_get('zofui_groupshop_addorder',array('orderid'=>$params['tid']));
				$_W['uniacid'] = $orderinfo['uniacid'];


				if($orderinfo['status'] == 0  && $orderinfo['totalmoney']*1 == $params['fee']*1){
					
					//改变订单状态
					$res = pdo_update('zofui_groupshop_addorder',array('status'=>1,'uorderid'=>$params['tag']['transaction_id']),array('id'=>$orderinfo['id']));
					
					if($res){
						$money = $orderinfo['totalmoney'] + $orderinfo['givemoney'];
						model_user::updateUserMoney($orderinfo['openid'],$money,2);
						Message::addmoneymessage($orderinfo['openid'],$module,$orderinfo['totalmoney'],$orderinfo['orderid'],$orderinfo['givemoney']);
					}					
				}
...
//			var_dump($order);die;
				if( !empty( $order ) ) {
					...
					message('充值完成',Util::createModuleUrl('user'),'success');
				}
				...
				if( $params['result'] == 'failed' && $params['type'] == 'delivery' && !empty($params['tid']) ){

					$orderinfo = $order = model_order::getSingleOrderAndGood($params['tid']); //查询订单
					
					if( !empty( $orderinfo ) ) {
						$_W['uniacid'] = $orderinfo['uniacid'];

						if($orderinfo['status'] == 1  && $orderinfo['totalmoney'] == $params['fee']){
							self::dealOrder($orderinfo,$params,$module,2);


							if($order['ordertype'] == 1){
								
								message('已提交订单！',Util::createModuleUrl('user',array('op'=>'orderinfo','id'=>$order['idoforder'])),'success');
							}else{
								if($order['ordertype'] == 2) $notice = '参团成功!';
								if($order['ordertype'] == 3) $notice = '开团成功!';
								...
								//之所以这里跳到orderinfo页面是因为，若是余额支付的话，执行此方法的时候团还没有插入到数据中，查询不到团,orderinfo页面做了跳转
								message($notice,Util::createModuleUrl('user',array('op'=>'orderinfo','gogroup'=>1,'id'=>$order['idoforder'])),'success');
							}


						}
					}else{
						message('支付失败！',Util::createModuleUrl('user',array('op'=>'orderinfo','id'=>$order['idoforder'])), 'error');
					}
	...
	static function dealOrder($orderinfo,$params,$module,$type){
		global $_W;
