...
class model_order
{
	...
	static function getSingleOrderAndGood($orderid){
		...
		$sql = " SELECT $select FROM ". tablename('zofui_groupshop_order') . " AS a RIGHT JOIN ". tablename('zofui_groupshop_ordergood') ." AS b ON a.`id` = b.idoforder LEFT JOIN ". tablename('zofui_groupshop_good') . " AS c ON c.`id` = b.`gid` WHERE a.orderid = :orderid ";
		$data = pdo_fetchall($sql,array(':orderid'=>$orderid));
		foreach($data as $k=>$v){
			if($k == 0){
				$order = $v;
			}
	...
	static function getAllOrder($wherea,$whereb,$statusstr,$select,$page,$num,$order,$isNeedpage){
		...
		$commonstr = tablename('zofui_groupshop_order') ." AS a LEFT JOIN ".tablename('zofui_groupshop_ordergood')." AS b ON a.`id` = b.`idoforder` WHERE ".$dataa[0].$datab[0].$str ;
	...
	static function cancelDoNotPayOrder($orderinfo,$module){
		global $_W;
		if($orderinfo['status'] != 1 && $orderinfo['status'] != 3) return false; //不是待支付的订单
		$res = pdo_update('zofui_groupshop_order',array('status'=>2,'canceltime'=>time()),array('id'=>$orderinfo['id']));
		...
		//恢复商品库存
		$ordergood = Util::getAllDataInSingleTable('zofui_groupshop_ordergood',array('idoforder'=>$orderinfo['id']),1,110,'id DESC',false,' gid,buynum ');
		...
		Message::imessage($orderinfo['openid'],$module,$orderinfo['orderid'],$orderinfo['totalmoney'],$orderinfo['id'],$orderinfo['title']); //发消息
	...
	static function refundMoney($id,$money,$module,$from){
		...
		if($orderinfo['paytype'] == 'credit'){ //余额支付
			...
			if($data['result_code']=='SUCCESS'){
				...
				$res['status'] = false;
				$res['error_code'] = $data['err_code_des'].$data['return_msg'];
			}
		
		}elseif($orderinfo['paytype'] == 'delivery'){ //货到付款
			$res['status'] = true;
		}
	...
	*/
	...
	static function countOrderMoney($orderInSession,$iscredit,$cardid,$province,$module,$sendtype = 0){
		...
		foreach($orderInSession as $k=>$v){
			...
			if( $goodinfo[$k]['isexpress'] == 1 ){
				$isexpress = 0;
			}
		}
		
		if( $selftake == 0 && $isexpress == 0 ){
			$isexpress = 1;
		}

		//计算邮费 之所以拿出来计算是因为需要用到订单总金额 $totalmoney[0]
		foreach($goodinfo as $k=>$v){
			if( $v['isexpress'] == 1 ) continue;

			$goodinfo[$k]['buyexpressmoney'] = model_express::countExpressMoney($v,$v['buynumber'],$province); //计算当前商品需要的邮费
			
			if( ($v['isfreeexpress'] == 1 && $v['fullmoney'] < $totalmoney[0] && $goodinfo[$k]['buyexpressmoney'] > 0) || ($module->module['config']['cratfreeex'] > 0 && $module->module['config']['cratfreeex'] < $totalmoney[0] && $goodinfo[$k]['buyexpressmoney'] > 0) || ( $module->module['config']['extype'] == 1 && $totalmoney[0] > $module->module['config']['itfee'] && count($goodinfo) > 1 ) ){
		...
		// 设置单独的运费 未满额包邮的情况下
		if(  !($module->module['config']['cratfreeex'] > 0 && $module->module['config']['cratfreeex'] < $totalmoney[0]) && $module->module['config']['extype'] == 1 && $totalmoney[0] > $module->module['config']['itfee'] && count($goodinfo) > 1 ){
			$totalexpressmoney = $module->module['config']['exfee'];
			$isoneexpress = 1;
		}
