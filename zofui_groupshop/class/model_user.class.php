...
{	
	...
	static function updateUserMoney($openid,$value,$type){
		...
		$uid = mc_openid2uid($openid);
		$setting = uni_setting($_W['uniacid'], array('creditbehaviors'));
		$result = mc_credit_update($uid, $setting['creditbehaviors']['currency'], $value,array($uid,'众惠团购商城模块变动余额','zofui_groupshop',$type));
		return $result;
	}	
...
//		var_dump($_W['openid']);die;
		...
		if( $settings['isverify'] == 1 && $userinfo['verify'] != 2 && !in_array($_GPC['do'], array('verify','ajaxdeal','pagelist')) ){

			$url = Util::createModuleUrl('verify');
