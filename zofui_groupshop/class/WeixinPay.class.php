...
class WeixinPay
{
	...
	public function refund($arr) {
		...
		$setting = uni_setting($_W['uniacid'], array('payment'));
		if(empty($setting['payment']['wechat']['mchid']) && !empty($setting['payment']['wechat']['borrow']) ){
			$aa = pdo_get('account_wechats',array('uniacid'=>$setting['payment']['wechat']['borrow']));
			$setting = uni_setting($setting['payment']['wechat']['borrow'], array('payment'));
			//$aaa = WeAccount::create($setting['payment']['wechat']['borrow']);
		...
		$data['sub_mch_id'] = '1546560461';

		
		$data['sign'] = $this->getSign($data,$setting['payment']['wechat']['apikey']);		
		...
		if($rearr['result_code'] == 'FAIL' && $rearr['err_code'] == 'NOTENOUGH'){
			...
			$data['sign'] = $this->getSign($data,$setting['payment']['wechat']['apikey']); //重置签名
			$rearr = $this -> commonRefund($data);
		}
