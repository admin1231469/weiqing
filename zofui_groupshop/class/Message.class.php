...
{{remark.DATA}}
	*/
	public static function imessage($openid,$module,$orderid,$fee,$idoforder,$title) {
		$url2 = Util::createModuleUrl('user',array('op'=>'orderinfo','id'=>$idoforder));
	...
	*/
    ...
    {{remark.DATA}}
	*/
	public static function gmessage($openid,$module,$title,$fullnumber,$groupid,$money) {
	...
	*/
	public static function emessage($openid,$module,$title,$expressname,$expressnumber,$address,$idoforder,$orderid) {
		$url2 = Util::createModuleUrl('user',array('op'=>'orderinfo','id'=>$idoforder));
		
		if(empty($expressname)) $expressname = '无物流';
		if(empty($expressnumber)) $expressnumber = '无物流';
		$i_item = '您的宝贝已上路，点击此处可查看订单详情';
...
{{remark.DATA}}
	*/
	public static function dmessage($openid,$module,$orderid,$idoforder,$money) {
...
{{remark.DATA}}
	...
	*/
	...
	// 充值提醒
	public static function addmoneymessage($openid,$module,$fee,$orderid,$givemoney) {
		$url2 = Util::createModuleUrl('user');
		$paytype = '微信支付';
		
		$title = '充值余额';
		$i_item = '充值成功，已发放到你的余额中';
		$i_id = $module -> module['config']['a_id'];
		$i_remark = $givemoney > 0 ? '充值获得赠送'.$givemoney.'余额' : '';
		$msg_json = '{
           	"touser":"' . $openid . '",
           	"template_id":"' . $i_id . '",
           	"url":"' . $url2 . '",
