...
/************以下是自动处理订单*****************/	
	...
	static function autoDealOrder($module){
		...
		foreach($orderdata[0] as $k=>$v){
            $info = pdo_fetch("SELECT * FROM " . tablename('zofui_groupshop_ordergood') . " WHERE idoforder=:idoforder LIMIT 1", array(':idoforder' => $v['id']));
			$v['title'] = $info['title'];
		...
		if($module->module['config']['remindmessagetime'] > 0){
			...
			foreach($orderdata[0] as $k=>$v){
                $info = pdo_fetch("SELECT * FROM " . tablename('zofui_groupshop_ordergood') . " WHERE idoforder=:idoforder LIMIT 1", array(':idoforder' => $v['id']));
				$res = Message::kmessage($v['openid'],'',$module,$v['totalmoney'],$info['title'],$v['createtime'],$v['orderid'],$v['id']);
	...
	function checkGroup($module){
		...
		foreach($data[0] as $k => $v){
			...
			foreach($orderdata[0] as $kk=>$vv){
				//发失败通知
				Message::gmessage($vv['openid'],$module,$v['title'],$v['fullnumber'],$v['id'],$vv['totalmoney']);
				//自动退款
...
/*************以下是发消息******************/	
	...
	public function sendMessage($module){
		...
		foreach($message as $k=>$v){
			...
			if($v['type'] == 5){ //发给管理员的下单通知
				$id = intval($v['str']);
				$order = Util::getSingelDataInSingleTable('zofui_groupshop_order',array('id'=>$id),' totalmoney,orderid');
				Message::jmessage($module,$order['orderid'],$order['totalmoney'],$id);
			}
	...
	public function sendgoodMessage($id,$module){
		...
		$order = Util::getSingelDataInSingleTable('zofui_groupshop_order',array('id'=>$id),'openid,name,tel,expressname,expressnumber,orderid');
		if(empty($order)) return false;
		Message::emessage($order['openid'],$module,$order['name'],$order['tel'],$order['expressname'],$order['expressnumber'],$id,$order['orderid']);
	}	
	...
	public function GroupDoneMessage($groupid,$module){
		...
		$sql = "SELECT openid,orderid,id,totalmoney FROM ".tablename('zofui_groupshop_order')." WHERE `uniacid` = :uniacid AND `status` = :status AND `groupid` = :groupid ORDER BY `id` DESC";
		...
		foreach($openidarray as $k=>$v){
			Message::dmessage($v['openid'],$module,$v['orderid'],$v['id'],$v['totalmoney']);
		}
