$(function(){
	...
	$('.add_a_ex').click(function(){
		var name = $(this).prev().find('input').val();
		if( name == '' ) return false;
		var str = '<div class="express_item">'+name+'<input type="hidden" name="exname[]" value="'+name+'"></div>';
		$('.express_box').append( str );
		$(this).prev().find('input').val('');
	});

	$('body').on('click','.express_item',function(){
		$(this).remove();
	});
...
/*添加编辑商品*/ // ///////////////////////////
	...
	$('.add_a_rule').click(function(){
		...
		if(type == '1'){
			...
			var pstr = ' 团购价: &nbsp; <span class="input_box input_box_100"> <input type="text" class="input_input" name="diffgroupprice[]"> </span>元 ';
			if( shoptype == 2 ) pstr = '';
	...
	$('form[name=addgood_form]').submit(function(){
		...
		if( form.isexpress == 1 || form.isselftake <= 0 ){
			alert('快递配送和上店自提必须开启一个');return false;
		}
	...
	changeItem($('input[name=isselftake]'),$('.isselftake'));
	...
	//添加充值赠送
	$('.add_a_addmoney').click(function(){
		var thisparent = $(this).parents('.good_params_item');
		var name = thisparent.find('input[name=addnum]').val();
		var pro = thisparent.find('input[name=givemoney]').val();		
		if(name == '' || pro == ''){
			alert('请先在左方填好充值金额和赠送金额');return false;
		}
		var str = 
				'<div class="good_params_item">'
					+'<div class="params_name_div">'
						+'<span class="input_box input_box_100">'
							+'<input type="text" class="input_input" name="paramsadd[]" value="'+name+'" >'
						+'</span>'
					+'</div>'
					+'<div class="params_pro_div">'
						+'<span class="input_box input_box_300">'
							+'<input type="text" class="input_input" name="paramsgive[]" value="'+pro+'" >'
						+'</span>'
						+'<a href="javascript:;" class="a_href deletethis_params"> 删除</a>'
					+'</div>'
				+'</div>';	
		$('.good_params_list').append(str);
		$('input[name=addnum]').val('').focus();
	...
	$('.cancela').click(function(){
		var thiselem = $(this);
		var thisid = thiselem.attr('oid');
		
		if( confirm('确定取消订单吗？') ){
			Http('post','html','cancela',{oid:thisid},function(data){
				if(data == 1){
					alert('已取消');
		...
		if(type == 'delete'){ //删除订单
			...
			if(confirm('确定要删除此订单吗？')){
				Http('post','html','deleteOrder',postdata,function(data){
					if(data == 1){
						alert('已删除此订单');
						location.href = window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op=list&do=order&m=zofui_groupshop';
					}else{
						alert(data);
					}
				},true);
	...
	$('.group_allrefund').click(function(){
		...
		Http('post','html','refundgroup',{id:id},function(data){
			if(data == 1){
				alert('已提交给计划任务处理，等待系统处理退款。(若没有设置计划任务请尽快按教程设置)');
				location.href = "";
			}else{
				alert(data.data);
			}
	...
	$('.search_admin').click(function(){
		...
		Http('post','json','getadmin',{nickname:nickname},function(data){
			if(data.status == 1){
				$('input[name=adminnickname]').val(data.data.nickname);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function ajaxUrl(op){
		return window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op='+op+'&do=ajax&m=zofui_groupshop';
	}
